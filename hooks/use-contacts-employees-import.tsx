...
export type UseContactsEmployeesImportResult = {
  progress: Progress;
  results: RowResult[];
  running: boolean;
  startImport: () => void;
 ...
 * @param rows Array of mapping rows with repCode and contactCode.
 */
export function useContactsEmployeesImport(
  rows: MappingRow[]
): UseContactsEmployeesImportResult {
  ...
  const processFn = async (row: MappingRow): Promise<RowResult> => {
    ...
    formData.append("contactCode", row.contactCode);

    const response = await fetch("/api/tedis/contacts-employees", {
      method: "POST",
      body: formData,
    });
